...
// Fetch data
$sql = "SELECT * FROM tbl_certificate WHERE status = 'Verified'";
...
<body>
                            ...
                            <tr>
                                ...
                                <td class="border-1" >
                                    <button data-id="<?php echo $certificate['rqst_id'] ?>"
                                        class="btn btn-success RegapproveCert">+</button>

                                    <button data-id="<?php echo $certificate['rqst_id'] ?>"
                                        class="btn btn-danger RegdeclineCert">-</button>
                                </td>
                            </tr>
                            <?php $count++; endwhile ?>
    ...
    <script>
        ...
        $(".RegapproveCert").click(function (e) {
            e.preventDefault();
            let id = $(this).data('id')

            $.ajax({
                type: "post",
                url: "../../backend/scripts/ajax.php?action=RegapproveCert&id=" + id,
        ...
        $(".RegdeclineCert").click(function (e) {
            e.preventDefault();
            let id = $(this).data('id')

            $.ajax({
                type: "post",
                url: "../../backend/scripts/ajax.php?action=RegdeclineCert&id=" + id,
                success: function (response) {
                    // alert(response)
                    if (response == 1) {
